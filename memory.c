...
if (match==true){
    ...
    allo_memory.start_address = temp_memory_block.start_address;
    allo_memory.end_address = allo_memory.start_address + request_size - 1;
    allo_memory.process_id = process_id;
    allo_memory.segment_size = request_size;
    *map_cnt = *map_cnt + 1;

    for(int i = memory_map_index; i <= *map_cnt; i++){
        memory_map[memory_map_index+1] = memory_map[i];
    }
    memory_map[memory_map_index+1].start_address = allo_memory.end_address +1;
    ...
    memory_map[memory_map_index] = allo_memory;
}
return allo_memory;
}
